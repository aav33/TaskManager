...
async function initializeData() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Tallennetaan kategoriat ja niiden ID:t muistiin
            categories = result.categories.map(c => {
                categoryMap[c.name] = c.id;
                return c.name;
            });

            // Muunnetaan tehtävät JS-muotoon
            ...
            if (categories.length > 0) {
                ...
                if(titleEl) titleEl.innerText = "Luo uusi minitaulu aloittaaksesi";
            }
        }
    } catch (error) {
...
async function openShareModal() {
    try {
        ...
        if (result.success && result.board.code) {
            ...
            alert("Jakokoodia ei löytynyt. Päivitä sivu kokeillaksesi uudelleen.");
        }
    } catch (error) {
...
async function sendCodeViaEmail() {
    ...
    if (!email || !email.includes('@')) {
        alert("Ole hyvä ja syötä kelvollinen sähköpostiosoite.");
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            alert("Koodi lähetettiin onnistuneesti osoitteeseen: " + email);
            emailInput.value = ""; 
            closeModal('share-modal');
        } else {
            alert("Virhe lähetettäessä: " + (result.error || "Palvelinvirhe"));
        }
    } catch (error) {
...
function switchCategory(cat) {
    currentCategory = cat;
    const titleEl = document.getElementById('current-category-title');
    if(titleEl) titleEl.innerText = cat || "Valitse tai luo kategoria";
    renderSidebar();
    renderTodos();
}
